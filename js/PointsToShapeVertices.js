class PointsToShapeVertices {
    ...
    for (let output_length = 1; true; output_length++) {
      ...
      for (let i = 0; i < inputs.length; ++i) {
        ...
        if (
          c > 0 ||
          (c === 0 &&
            this.aIsFar(outputs[output_length - 1], inputs[i], inputs[next]))
        ) {
          next = i;
        }
      }
  ...
  addNoise() {
    ...
    for (let i = 0; i < this.points.length; i++) {
      ...
      let newPoint = this.points[i]
        .clone()
        .add(this.points[(i + 1) % this.points.length].clone())
        .divideScalar(2);
      newPoint.add(
        newPoint
          .clone()
  ...
  relocatePointOutOfCanvas(canvas) {
    ...
    this.points.forEach((point) => {
      ...
      else if (point.x < -width / 2) point.x = -width / 2;

      if (point.y > height / 2) point.y = height / 2;
      else if (point.y < -height / 2) point.y = -height / 2;
    });
  }
}
