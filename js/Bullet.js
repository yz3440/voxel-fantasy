class Bullet {
  ...
  generatePointCloud(resolution) {
      ...
      for (let h = 0; h < 2; h++) {
        for (let i = 0; i < 4; i += 2) {
          this.pointCloud.push([
            this.vertices2D[h][i].clone(),
            this.vertices2D[h][1].clone(),
          ]);
          this.pointCloud.push([
            this.vertices2D[h][i].clone(),
            this.vertices2D[h][3].clone(),
          ]);
        }
      }
      for (let i = 0; i < 4; i += 1) {
        this.pointCloud.push([
          this.vertices2D[0][i].clone(),
          this.vertices2D[1][i].clone(),
        ]);
      }
    }
    for (let j = 0; j < this.pointCloud.length; j++) {
      ...
      for (let i = 1; i < resolution; i++) {
        ...
        this.pointCloud[j].push(
          new THREE.Vector3(
            factor * x_1 + rFractor * x_2,
            factor * y_1 + rFractor * y_2,
            factor * z_1 + rFractor * z_2
          )
        );
      }
    }
  ...
  checkNearCanvas(canvas) {
    this.isNearCanvas =
      Math.abs(canvas.plane.distanceToPoint(this.pos)) < (this.size / 2) * 1.5;
