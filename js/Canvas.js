class Canvas {
  constructor(index, scene, sizeX, sizeY, posX, posY, posZ, color) {
    ...
    this.frameObj = new THREE.LineSegments(
      this.edgesGeometry,
      this.edgeMaterial
    );
    ...
    this.reverseEuler = new THREE.Euler(
      -rotationX,
      -rotationY,
      -rotationZ,
      'XYZ'
  ...
  updateTranslation(controlHand) {
    if (controlHand.hasHandOnce) {
      ...
      let lerpSpeed = 0.4;
      this.frameObj.rotation.x = THREE.Math.lerp(
        this.frameObj.rotation.x,
        rotationX,
        lerpSpeed
      );
      this.surfaceObj.rotation.x = THREE.Math.lerp(
        this.surfaceObj.rotation.x,
        rotationX,
        lerpSpeed
      );
      this.frameObj.rotation.y = THREE.Math.lerp(
        this.frameObj.rotation.y,
        rotationY,
        lerpSpeed
      );
      this.surfaceObj.rotation.y = THREE.Math.lerp(
        this.surfaceObj.rotation.y,
        rotationY,
        lerpSpeed
      );
      this.frameObj.rotation.z = THREE.Math.lerp(
        this.frameObj.rotation.z,
        rotationZ,
        lerpSpeed
      );
      this.surfaceObj.rotation.z = THREE.Math.lerp(
        this.surfaceObj.rotation.z,
        rotationZ,
        lerpSpeed
      );
    }
    ...
    bullets.forEach((bullet) => {
      ...
      if (bullet.isNearCanvas) {
        ...
        if (collidingPoints.length > 0) {
          ...
          else this.addImprints(collidingPoints, bullet.color);
        }
      }
    });
  ...
  checkCollision(bullet) {
    ...
    pointCloud.forEach((points) => {
      points.forEach((point) => {
        let pointToCenter = new THREE.Vector3(
          point.x - this.pos.x,
          point.y - this.pos.y,
          point.z - this.pos.z
        );
  ...
  addImprints(collidingPoints, color) {
    ...
    let imprintMaterial = new THREE.MeshBasicMaterial({
      color: color,
      side: THREE.DoubleSide,
    });
