class ControlHand {
  ...
  update(res) {
    // if res is undefined, leap motion is not connected
    ...
    res.forEach((hand) => {
      if (hand.type === this.type) {
        ...
        if (hand.grabStrength === 0) {
          ...
          let currentNormalVector = new THREE.Vector3(
            hand.palmNormal[0],
            hand.palmNormal[1],
            hand.palmNormal[2]
          );
          let currentDirectionVector = new THREE.Vector3(
            hand.direction[0],
            hand.direction[1],
            hand.direction[2]
          );
          ...
          if (
            (this.prevHasHand &&
              currentNormalVector.angleTo(this.normalVector) <
                updateThreshold) ||
            !this.prevHasHand
          ) {
